...
Grafana Cloud에서 발급한 external ID는 local `staging.tfvars`의 `external_id`에 설정합니다. 이 파일은 Git에서 제외되며, tracked `staging.tfvars.example`에는 placeholder만 둡니다. 기본 역할명은 `iam_role_name = "GrafanaCloudWatchIntegration"`이며 필요 시 변경할 수 있습니다. apply 후 `terraform output -raw grafana_cloudwatch_role_arn`의 ARN을 Grafana Cloud CloudWatch integration에 등록합니다. external ID는 IAM trust policy의 일부이므로 Terraform state에는 포함될 수 있습니다. state backend와 state를 읽을 수 있는 IAM principal은 승인된 운영자로 제한합니다. Terraform은 Grafana Labs AWS account에만 이 역할을 assume하도록 제한하며, trust policy의 external ID 조건도 함께 검증합니다.
