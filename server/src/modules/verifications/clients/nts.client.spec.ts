...
describe('NtsClient', () => {
  ...
  it('posts the registration number to the status endpoint and parses a match', async () => {
    ...
    expect(result).toEqual({ valid: true, status: '계속사업자' });
  ...
  it('rejects mismatched and closed registrations', async () => {
    const client = new NtsClient();
    respondWith({ data: [{ b_no: '1', valid: '02', b_stt: '등록 정보 불일치' }] });
    await expect(client.verifyBusinessRegistration('1', 'x')).resolves.toEqual({
      valid: false,
      status: '등록 정보 불일치',
    });

    respondWith({ data: [{ b_no: '1', valid: '01', b_stt: '폐업자', b_stt_cd: '03' }] });
    await expect(client.verifyBusinessRegistration('1', 'x')).resolves.toEqual({
      valid: false,
      status: '폐업자',
    });
