...
const PopupAction = styled('button', { shouldForwardProp: (prop) => prop !== 'secondary' })<{ secondary?: boolean }>(({ secondary }) => ({ height: 37, border: secondary ? `1px solid ${c.gray200}` : 0, borderRadius: 6, background: secondary ? c.white : c.primary, color: secondary ? c.gray900 : c.white, cursor: 'pointer', fontSize: secondary ? 12 : 13, fontWeight: 600, lineHeight: secondary ? 'normal' : 1.4, '&:disabled': { opacity: 0.4, cursor: 'not-allowed' } }));
const NameBackdrop = styled.div({ position: 'fixed', zIndex: 100, inset: 0, display: 'grid', placeItems: 'center', padding: 24, background: 'rgba(17, 17, 17, .2)' });
const NamePopup = styled.div({ width: 'min(100%, 427px)', padding: 30, borderRadius: 9, background: c.white, boxShadow: '0 20px 48px rgba(17, 24, 39, .22)', display: 'flex', flexDirection: 'column', gap: 16 });
const NameFields = styled.div({ display: 'flex', flexDirection: 'column', gap: 8 });
const NameInput = styled.input({
  width: '100%', height: 44, padding: '0 14px', border: `1px solid ${c.gray200}`, borderRadius: 8, background: c.white, color: c.gray900, ...textStyle.body,
...
const PositionSelect = styled.select({
  width: '100%', height: 44, padding: '0 34px 0 14px', border: `1px solid ${c.gray200}`, borderRadius: 8, background: c.white, color: c.gray900, cursor: 'pointer', ...textStyle.body, appearance: 'none',
  '&:focus': { outline: 'none', borderColor: c.primary },
});
const PositionIcon = styled.img({ position: 'absolute', top: '50%', right: 12, width: 14, height: 14, transform: 'translateY(-50%)', pointerEvents: 'none' });
