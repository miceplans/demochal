...
export function BizAdsPage() {
  ...
  const [pendingRange, setPendingRange] = useState<DateRange | undefined>({
    from: new Date('2026-08-25T00:00:00'),
    to: new Date('2026-08-27T00:00:00'),
  });
  ...
  const paymentDays = Math.max(
    1,
    Math.round(
      (new Date(`${paymentEnd}T00:00:00`).getTime() -
        new Date(`${paymentStart}T00:00:00`).getTime()) /
        86_400_000,
    ),
  );
      ...
      <ManageBody>
        ...
        <ManageSection aria-labelledby="ad-management-title">
          ...
          <AdsTable role="table" aria-label="광고 관리 목록">
            ...
            {myAds.map((ad) => (
              <TableRow key={ad.title} role="row">
                <span role="cell">{ad.title}</span>
                <span role="cell">{ad.price}</span>
                <span
                  role="cell"
                  style={{
                    justifySelf: 'end',
                    width: 80,
                    color: ad.status === '진행중' ? c.primary : c.red,
                  }}
                >
                  {ad.status}
                </span>
    ...
    return (
      <CheckoutBody>
        <HeaderRow>
          ...
          <h1 style={{ margin: 0, ...textStyle.h1_2 }}>광고 결제</h1>
        </HeaderRow>
        <CheckoutCard>
          <h2 style={{ margin: 0, ...textStyle.h2_2 }}>선택한 광고</h2>
          <CheckoutRow>
            <span>{selectedAd.name}</span>
            <strong>{selectedAd.price.toLocaleString()}원</strong>
          </CheckoutRow>
          <CheckoutRow>
            <span style={{ color: c.gray500 }}>노출 기간</span>
            <span>{selectedAd.period}</span>
          </CheckoutRow>
        </CheckoutCard>
        <CheckoutCard>
          <h2 style={{ margin: 0, ...textStyle.h2_2 }}>결제수단</h2>
          <PaymentMethod>
            <span>
              <strong>등록된 카드</strong>
              <span style={{ marginLeft: 10, color: c.gray500 }}>
                <MaskedText
                  value={paymentCard.number}
                  masked={maskCardNumber(paymentCard.number)}
                />
              </span>
            </span>
            <span style={{ color: c.primary, ...textStyle.label }}>선택됨</span>
          </PaymentMethod>
        </CheckoutCard>
        <CheckoutCard aria-label="결제 금액">
          <CheckoutRow>
            <span style={{ color: c.gray500 }}>상품 금액</span>
            <span>{selectedAd.price.toLocaleString()}원</span>
          </CheckoutRow>
          <CheckoutRow>
            <span style={{ color: c.gray500 }}>부가세</span>
            <span>0원</span>
          </CheckoutRow>
          <Divider />
          <CheckoutRow>
            <strong>총 결제 금액</strong>
            <strong style={{ fontSize: 22, color: c.primary }}>
              {selectedAd.price.toLocaleString()}원
            </strong>
          </CheckoutRow>
        </CheckoutCard>
        <PrimaryButton
        ...
        >
          {selectedAd.price.toLocaleString()}원 결제하기
        </PrimaryButton>
      </CheckoutBody>
    );
    ...
    return (
      <CheckoutBody style={{ alignItems: 'center', paddingTop: 110 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <h1 style={{ margin: 0, ...textStyle.h1_2 }}>광고 결제가 완료되었습니다</h1>
          <p style={{ margin: '10px 0 0', color: c.gray500, ...textStyle.body }}>
            {selectedAd.name}가 {selectedAd.period} 동안 노출됩니다.
          </p>
        </div>
