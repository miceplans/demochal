...
export function AdPlacementPreview({
  ...
  return (
    <Canvas ref={canvasRef} view={view} aria-label={`${view === 'mobile' ? '모바일' : 'PC'} 사용자 홈 광고 미리보기`}>
      <MovingAds ariaLabel="홈 상단 광고" itemCount={5} interval={5000} paused={uploadPlacement === 'hero'}>
              ...
              {loopIndexes.map((item, position) => {
                if (uploadPlacement === 'hero') {
                  return (
                    <HeroSlotFrame key={`${item}-${position}`} view={view}>
                      <AdImageUploader compact={view === 'mobile'} busy={processing} onFileSelected={(file) => onImagePicked?.('hero', file)} />
                    </HeroSlotFrame>
                  );
                }
                ...
                return (
                  <Slot key={`${item}-${position}`} type="button" active={active === 'hero'} onMouseEnter={handleSlotEnter('hero')} onFocus={showTooltip('hero')} onBlur={() => setTooltip(null)} onClick={() => handleSlotClick('hero')} aria-label="홈 상단 광고 선택">
                    <img src={uploaded ?? '/assets/figma-ads/home-hero.png'} alt={uploaded ? '업로드한 홈 상단 광고 이미지' : position === 1 ? '홈 상단 광고 예시' : ''} />
                  </Slot>
                );
              })}
            </HeroRail>
      ...
      <MovingAds ariaLabel="홈 중간 이미지 광고" itemCount={5} interval={5000} paused={uploadPlacement === 'gallery'}>
        {({ activeIndex, loopIndexes, railIndex, shouldAnimate, handleTransitionEnd }) => (
          <GalleryViewport>
              ...
              {loopIndexes.map((item, position) => {
                if (uploadPlacement === 'gallery') {
                  return (
                    <GallerySlotFrame key={`${item}-${position}`} view={view}>
                      <AdImageUploader compact busy={processing} onFileSelected={(file) => onImagePicked?.('gallery', file)} />
                    </GallerySlotFrame>
                  );
                }
                ...
                return (
                  <GallerySlot key={`${item}-${position}`} view={view} type="button" active={active === 'gallery'} onMouseEnter={handleSlotEnter('gallery')} onFocus={showTooltip('gallery')} onBlur={() => setTooltip(null)} onClick={() => handleSlotClick('gallery')} aria-label="홈 중간 이미지 광고 선택">
                    <img src={uploaded ?? '/assets/figma-ads/home-gallery.png'} alt={uploaded ? '업로드한 홈 중간 광고 이미지' : position === 1 ? '홈 중간 광고 예시' : ''} />
                  </GallerySlot>
                );
              })}
            </GalleryRail>
...
const HeroSlotFrame = styled('div', { shouldForwardProp: (prop) => prop !== 'view' })<{ view: AdPreviewView }>(({ view }) => ({
  position: 'relative', display: 'block', width: '100%', height: view === 'mobile' ? 150 : 'clamp(150px, 21vw, 252px)', padding: 8, overflow: 'hidden',
  border: `4px dashed ${c.primary}`, borderRadius: 20, background: c.white,
}));
...
const GallerySlotFrame = styled('div', { shouldForwardProp: (prop) => prop !== 'view' })<{ view: AdPreviewView }>(({ view }) => ({
  position: 'relative', display: 'block', flex: `0 0 ${view === 'mobile' ? 122 : 314}px`, padding: view === 'mobile' ? 3 : 8, overflow: 'hidden',
  border: `4px dashed ${c.primary}`, borderRadius: 20, background: c.white,
}));
