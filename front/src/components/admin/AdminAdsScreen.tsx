...
const TableRow = styled.div<{ header?: boolean }>(({ header }) => ({
  display: 'grid',
  gridTemplateColumns: '180px 180px 180px minmax(80px, 1fr)',
  alignItems: 'center',
  gap: 16,
  height: header ? 52 : 56,
  padding: '0 16px',
  borderTop: header ? 0 : '1px solid #DFE2E7',
  background: header ? c.gray100 : c.white,
  color: c.gray900,
  ...(header ? textStyle.h3_2 : textStyle.bodyLarge),
}));
...
export function AdminAdsScreen() {
  ...
  const rows = useMemo(
    () =>
      ads.filter(
        (ad) => `${ad.location} ${ad.title}`.includes(query) && (!status || ad.status === status),
      ),
    [query, status],
  );
  const handleActivity = (value: string) => {
    const label =
      value === '광고 수정하기' ? value : value === '내보내기' ? value : '광고 중단하기';
      ...
      <Table role="table" aria-label="광고 관리 목록">
        ...
        {rows.map((ad) => (
          <TableRow key={ad.id} role="row">
            <span role="cell">{ad.location}</span>
            <span role="cell">{ad.title}</span>
            <span role="cell">{ad.price}</span>
            <Progress role="cell">{ad.status}</Progress>
          </TableRow>
        ))}
      </Table>
      <Preview>
