...
const MemberName = styled(Input)({
  flex: 1,
  height: 40,
  [mobile]: { height: 44, textAlign: 'center', borderRadius: 10, padding: '0 12px' },
});
    ...
    <UserShell title="지원서 작성" back="/contests/public-data" footer={false}>
      ...
      >
        <Content>
          ...
          <Step>
            <StepHeading>
              <StepNumber $completed={membersComplete}>2</StepNumber>
              ...
              {members.map((member, index) => (
                <MemberRow key={index}>
                  ...
                  <MemberName
                    ...
                    placeholder="이름"
                    value={member.name}
                    style={{ color: member.name ? c.gray900 : c.gray500 }}
                    onChange={(event) =>
                      setMembers(
                        members.map((item, itemIndex) =>
